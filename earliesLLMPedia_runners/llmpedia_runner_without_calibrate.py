...
# ---------- Elicitation calibrate-parsing ----------
def _parse_calibrate_article(raw) -> dict:
    """
    Parse calibrate-elicitation JSON:
...
def main():
    ...
    # NER threshold (only used if confidence is present)
    ap.add_argument(
        "--ner-conf-threshold",
        type=float,
        default=0.0,
    ...
    def _generate_article(subject: str, hop: int):
        try:
            ...
            # Optional footer injection (push category discipline, but actual prompt lives in JSON files)
            if args.footer_mode:
                if args.domain == "topic":
                    footer = (
                        ...
                        "technologies and concepts that are strongly associated with this entity AND relevant to the root topic.\n"
            ...
            calibrate_mode = (args.elicitation_strategy == "calibrate")
            ...
            if calibrate_mode:
                parsed = _parse_calibrate_article(resp)
                wikitext = parsed.get("wikitext") or f"'''{subject}'''\n\nNo article content generated."
                overall_conf = parsed.get("overall_confidence")
                cat_meta = parsed.get("categories") or []
            else:
                # NEW topic-style: plain Wikitext article (generated by a simple Wikipedia-style prompt)
            ...
            # 1) Wikilinks are the main source of new entities
            for t in links_from_markup:
                _add_candidate(t)

            # 2) Optional calibrate category metadata
            for c in cat_meta:
                if not isinstance(c, dict):
                    continue
                name = c.get("name")
            ...
            if args.debug:
                _dbg(
                    ...
                    f"cats_meta={[cm.get('name') for cm in cat_meta][:10] if cat_meta else []}"
                )
            ...
            if candidates_for_ner:
                ...
                for d in decisions:
                    ...
                    # threshold logic (only if we have a confidence and threshold > 0)
            ...
            if next_subjects:
                ...
                for s in next_subjects:
                    ...
                    # avoid duplicating in this process (queue also dedupes)
